<template>
  <div>
    ...
    <div class="table">
        ...
        <el-table :data="tableData"
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <a v-if="`${state}`==='one'"
                class="address"
                @click="relationEdit(scope.row.projectId)">编辑</a>
              <span v-if="scope.row.editStatus==='0'&&`${state}`==='two'">
                <a class="address"
                  @click="relationEdit(scope.row.projectId)">编辑</a>
              </span>
            </template>
      ...
      <el-button :disabled="!status"
        type="primary"
        class="button"
        @click="submission">提交</el-button>
    </div>
...
import { projectDataGet, projectDataTypeGet, projectStatusGet, projectStatusEdit, appUserGet } from '../api.js';

export default {
  data() {
    const { projectId, datatype, state } = this.$route.query;
    return {
      // 关联选择
      relationOptions: [
        { value: '', label: '全部' },
  ...
  methods: {
    ...
    submission() {
      projectStatusGet({ projectId: this.projectId }).then(({ err, data: Data }) => {
        if (!err) {
          ...
          if (!_.isEmpty(ids)) {
            projectStatusEdit({ id: ids, status: '1' }).then(({ err: errs }) => {
              if (!errs) {
                this.$message({
                  message: '提交成功',
                  type: 'success',
    ...
    // 关联编辑
    relationEdit(projectId) {
      const status = this.state === 'one' ? '1' : '2';
      updateTag({
        name: '关联数据',
        cmd: 'addTag',
        to: `/datamap/relation/${status}?projectId=${projectId}&datatype=${this.datatype}`,
        appPath: window.location.pathname,
      });
    },
    /* 获取用户信息 */
